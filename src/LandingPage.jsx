              ...
              {menuOpen&&(
                <div style={{width:"100%",display:"flex",flexDirection:"column",gap:2,padding:"6px 0 10px",borderTop:`1px solid ${C.beigedk}`,marginTop:8}}>
                  ...
                  <button
                    ...
                    className="paw-btn"
                    style={{padding:"12px 14px",borderRadius:12,border:"none",background:C.cafe,color:C.white,fontWeight:800,fontSize:".95rem",cursor:"pointer",textAlign:"left",marginTop:4}}
                  >
                    Iniciar sesión 🔑
                  </button>
                </div>
...
function Carousel({onLoginClick, onOpenProfile}){
  ...
  const fallback=[
    {id: 901, emoji: E.dog, nombre: "Max", raza: "Labrador Retriever", edad: "2 años", tamano: "Mediano", sexo: "Macho", ubicacion: "CDMX", historia: "Max es juguetón, noble y aprende rápido. Busca una familia que disfrute paseos diarios.", foto_url: "https://images.unsplash.com/photo-1552053831-71594a27632d?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,${C.cafe},${C.cafeMd})`, estatus: "En adopción"},
    {id: 902, emoji: E.cat, nombre: "Mochi", raza: "Mestizo", edad: "1 año", tamano: "Chico", sexo: "Hembra", ubicacion: "Del Valle", historia: "Mochi es tranquila, curiosa y muy cariñosa cuando toma confianza. Ideal para depa.", foto_url: "https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,${C.cafeMd},${C.cafeLt})`, estatus: "En adopción"},
    {id: 903, emoji: E.poodle, nombre: "Luna", raza: "Poodle", edad: "3 años", tamano: "Chico", sexo: "Hembra", ubicacion: "Coyoacán", historia: "Luna ama estar cerca de las personas. Se adapta bien a hogares pacientes.", foto_url: "https://images.unsplash.com/photo-1548199973-03cce0bbc87b?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,${C.cafeLt},${C.cafeXlt})`, estatus: "En adopción"},
    {id: 904, emoji: E.catFace, nombre: "Canela", raza: "Siamés", edad: "2 años", tamano: "Chico", sexo: "Hembra", ubicacion: "Narvarte", historia: "Canela es observadora y dulce. Necesita un hogar seguro donde pueda explorar sin prisas.", foto_url: "https://images.unsplash.com/photo-1519052537078-e6302a4968d4?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,#8B4513,${C.cafeMd})`, estatus: "En adopción"},
    {id: 905, emoji: E.guideDog, nombre: "Thor", raza: "Pastor Alemán", edad: "4 años", tamano: "Grande", sexo: "Macho", ubicacion: "Condesa", historia: "Thor es leal y activo. Le van bien familias con espacio, estructura y experiencia.", foto_url: "https://images.unsplash.com/photo-1507146426996-ef05306b995a?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,${C.cafe},#A0522D)`, estatus: "En adopción"},
    {id: 906, emoji: E.smileCat, nombre: "Mina", raza: "Ragdoll", edad: "8 meses", tamano: "Chico", sexo: "Hembra", ubicacion: "Portales", historia: "Mina es sociable, dormilona y disfruta jugar con pelotas suaves.", foto_url: "https://images.unsplash.com/photo-1533743983669-94fa5c4338ec?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,${C.cafeLt},${C.cafe})`, estatus: "En adopción"},
    {id: 907, emoji: E.dogFace, nombre: "Coco", raza: "Chihuahua", edad: "5 años", tamano: "Chico", sexo: "Macho", ubicacion: "Nápoles", historia: "Coco es pequeño pero muy expresivo. Prefiere hogares tranquilos y compañía constante.", foto_url: "https://images.unsplash.com/photo-1583511655826-05700d52f4d9?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,${C.cafeXlt},${C.cafeMd})`, estatus: "En adopción"},
    {id: 908, emoji: E.blackCat, nombre: "Noche", raza: "British", edad: "2 años", tamano: "Chico", sexo: "Macho", ubicacion: "San Rafael", historia: "Noche es independiente, limpio y afectuoso a su ritmo.", foto_url: "https://images.unsplash.com/photo-1592194996308-7b43878e84a6?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,#3A2010,${C.cafe})`, estatus: "En adopción"},
    {id: 909, emoji: E.serviceDog, nombre: "Rex", raza: "Golden Retriever", edad: "3 años", tamano: "Grande", sexo: "Macho", ubicacion: "Santa María", historia: "Rex es noble y sociable. Disfruta pasear, aprender comandos y convivir con personas activas.", foto_url: "https://images.unsplash.com/photo-1587300003388-59208cc962cb?auto=format&fit=crop&w=900&q=80", color: `linear-gradient(135deg,${C.cafeMd},${C.cafeXlt})`, estatus: "En adopción"},
  ];
  ...
  useEffect(()=>{
    ...
    fetch(apiUrl("animales.php?action=public"))
      .then(r=>r.json())
      .then(d=>{
        const liveAnimals = (d.ok && Array.isArray(d.animals)) ? d.animals.filter(a=>a.estatus!=="Adoptado") : [];
        const map = new Map();
        const combined = [];
        for (const item of [...localSaved, ...liveAnimals]) {
  ...
  const rawItems=animals.length>0?animals:fallback;

  // State for animated filters
  const [filterSp, setFilterSp] = useState("all");
  const [filterSz, setFilterSz] = useState("all");

  // Filtering by species and size strictly in real-time
  const items = rawItems.filter(a => {
    const spRaw = (a.especie || a.species || "").toLowerCase();
    const razaRaw = (a.raza || a.kind || "").toLowerCase();

    const isCat = spRaw.includes("gato") || razaRaw.includes("gato") || razaRaw.includes("siamés") || razaRaw.includes("persa") || razaRaw.includes("ragdoll") || razaRaw.includes("british") || (a.emoji && (a.emoji.includes("🐈") || a.emoji.includes("🐱") || a.emoji.includes("😺")));
    const isDog = spRaw.includes("perro") || razaRaw.includes("perro") || razaRaw.includes("mestizo") || razaRaw.includes("chihuahua") || razaRaw.includes("labrador") || razaRaw.includes("golden") || razaRaw.includes("pastor") || (a.emoji && (a.emoji.includes("🐕") || a.emoji.includes("🐶") || a.emoji.includes("🐕‍🦺")));

    const sp = isCat ? "gato" : isDog ? "perro" : (spRaw || "perro");

    const szRaw = (a.talla || a.tamano || a.size || "mediano").toLowerCase().replace("ñ", "n").replace("chico", "pequeno");
    const sz = szRaw.includes("pequ") || szRaw.includes("chic") ? "pequeno" : szRaw.includes("grand") ? "grande" : "mediano";

    const matchSp = filterSp === "all" ? true : sp === filterSp;
    const matchSz = filterSz === "all" ? true : sz === filterSz;

    return matchSp && matchSz;
  });

  const displayItems = items;

  const row1=displayItems.filter((_,i)=>i%3===0);
  const row2=displayItems.filter((_,i)=>i%3===1);
  const row3=displayItems.filter((_,i)=>i%3===2);

  const buildBaseRow=(arr,min=6)=>{
    const base = arr.length > 0 ? arr : displayItems;
    if (base.length === 0) return [];
    const out=[...base];
    while(out.length<min)out.push(...base);
    return out.slice(0,Math.max(min,base.length));
  };
  ...
  const r1=b1.length>0?[...b1,...b1,...b1]:[];
  const r2=b2.length>0?[...b2,...b2,...b2]:[];
  const r3=b3.length>0?[...b3,...b3,...b3]:[];
  ...
  return(
    <section id="adoptar" className="collage-section" style={{...sectionTexture(C.beige),padding:"80px 0",overflow:"hidden",position:"relative"}}>
      ...
      <div style={{maxWidth:1240,margin:"0 auto",padding:"0 5%",marginBottom:20,display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexWrap:"wrap",gap:16}}>
        <div className="reveal">
          ...
          <h2 style={{fontFamily:"'Fredoka',sans-serif",fontWeight:700,fontSize:"clamp(1.9rem,3vw,2.8rem)",color:C.cafe,lineHeight:1.1}}>Te están esperando</h2>
        </div>
        <div className="reveal" style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",justifyContent:"flex-end"}}>
          <p style={{fontSize:".92rem",color:C.sub,maxWidth:340,lineHeight:1.75}}>Cada uno tiene una historia única. Quizás el tuyo está aquí.</p>
          <span style={{padding:"5px 10px",borderRadius:999,fontSize:".68rem",fontWeight:800,border:`1px solid ${mode==="live"?C.cafeXlt:"#F5D48B"}`,background:mode==="live"?`${C.cafe}12`:"#FFF7D6",color:mode==="live"?C.cafe:"#7A5200"}}>
            {mode==="live"?"Catálogo en vivo":"Catálogo demo"}
          </span>
        </div>
      </div>

      {/* Bar de Filtros Animados */}
      <div style={{maxWidth:1240,margin:"0 auto",padding:"0 5%",marginBottom:24,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
        {[["all","Todos 🐾"],["perro","Perros 🐕"],["gato","Gatos 🐈"]].map(([v,l])=>(
          <button key={v} onClick={()=>setFilterSp(v)} className="paw-btn" style={{
            padding:"8px 20px",borderRadius:999,
            border:`1.5px solid ${filterSp===v?C.cafe:C.beigedk}`,
            background:filterSp===v?C.cafe:C.cream,
            color:filterSp===v?C.white:C.sub,
            fontWeight:800,fontSize:".84rem",cursor:"pointer",
            boxShadow:filterSp===v?`0 4px 16px ${C.shadowMd}`:"none",
            transform:filterSp===v?"scale(1.05)":"scale(1)",
            transition:"all .25s cubic-bezier(0.34, 1.56, 0.64, 1)"
          }}>
            {l}
          </button>
        ))}
        <div style={{width:1,height:22,background:C.beigedk,margin:"0 2px"}}/>
        {[["all","Todas las tallas"],["pequeno","Pequeño"],["mediano","Mediano"],["grande","Grande"]].map(([v,l])=>(
          <button key={v} onClick={()=>setFilterSz(v)} className="paw-btn" style={{
            padding:"8px 16px",borderRadius:999,
            border:`1.5px solid ${filterSz===v?C.cafe:C.beigedk}`,
            background:filterSz===v?`${C.cafe}18`:C.cream,
            color:filterSz===v?C.cafe:C.muted,
            fontWeight:filterSz===v?800:600,fontSize:".8rem",cursor:"pointer",
            transform:filterSz===v?"scale(1.04)":"scale(1)",
            transition:"all .25s cubic-bezier(0.34, 1.56, 0.64, 1)"
          }}>
            {l}
          </button>
        ))}
        {(filterSp!=="all"||filterSz!=="all")&&(
          <button onClick={()=>{setFilterSp("all");setFilterSz("all");}} style={{padding:"7px 14px",borderRadius:999,border:`1px solid ${C.beigedk}`,background:C.cream,color:C.muted,fontWeight:700,fontSize:".76rem",cursor:"pointer"}}>
            Limpiar ×
          </button>
        )}
      </div>
      ...
      {displayItems.length === 0 ? (
        <div style={{maxWidth:1240,margin:"0 auto",padding:"30px 5%",textAlign:"center"}}>
          <div style={{background:C.cream,border:`1.5px dashed ${C.beigedk}`,borderRadius:20,padding:"36px 24px",boxShadow:`0 8px 24px ${C.shadow}`}}>
            <div style={{fontSize:"3.2rem",marginBottom:10}}>🐾</div>
            <h3 style={{fontFamily:"'Fredoka',sans-serif",fontSize:"1.4rem",color:C.cafe,fontWeight:700}}>
              No hay peluditos en esta categoría por el momento
            </h3>
            <p style={{fontSize:".88rem",color:C.sub,marginTop:6}}>
              Prueba ajustando los filtros o seleccionando "Todos".
            </p>
            <button
              onClick={()=>{setFilterSp("all");setFilterSz("all");}}
              className="paw-btn"
              style={{marginTop:16,padding:"10px 24px",borderRadius:999,border:"none",background:C.cafe,color:C.white,fontWeight:800,fontSize:".85rem",cursor:"pointer"}}
            >
...
function AnimalProfilePage({animal,onClose,waitDays=1,onNotify}){
      ...
      <div style={{maxWidth:1120,margin:"0 auto"}}>
        ...
        <div className="animal-profile-shell" style={{background:C.cream,borderRadius:26,boxShadow:"0 28px 80px rgba(22,83,187,.18)",border:`1.5px solid ${C.beigedk}`,overflow:"hidden"}}>
          ...
          <div className="animal-profile-details" style={{padding:"30px 30px 32px",position:"relative"}}>
            ...
            <button onClick={()=>setShowWizard(true)} className="paw-btn" style={{width:"100%",padding:"14px 16px",border:"none",borderRadius:14,background:C.cafe,color:C.white,fontWeight:900,fontSize:".98rem",cursor:"pointer",boxShadow:`0 10px 24px ${C.shadowMd}`}}>
      ...
      {showWizard && (
        <FormularioWizard
          ...
          onSuccess={(petName) => {
            setShowWizard(false);
            onClose();
            onNotify?.(`¡Solicitud enviada para ${petName}! El rescatista te contactará pronto 🐾`, "success");
          }}
        />
      )}
    </div>
...
function LoginModal({onClose,onLogin,onNotify}){
  ...
  const doLogin=async()=>{
    ...
    try {
      ...
      if (d && d.ok) {
        onNotify?.(`¡Inicio de sesión exitoso! 🐾`, "success");
        onLogin(d.user);
        setLoading(false);
        return;
      }
    ...
    let storedUsers = [];
    try { storedUsers = JSON.parse(localStorage.getItem("dogood_custom_users") || "[]"); } catch {}
    ...
    const loggedUser = matched || {
      ...
      email: email,
      rol: (email.includes("admin") || email.includes("dogood")) ? "admin" : "rescatista"
    };

    onNotify?.(`¡Bienvenido ${loggedUser.nombre}! 🐾`, "success");
    onLogin(loggedUser);
    setLoading(false);
  };
  ...
  const quickLogin=async type=>{
    ...
    try{
      ...
      for (const url of tryUrls) {
        try {
          const r = await fetch(url, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ email: m.e, password: m.p })
          });
          if (r.ok) {
            d = await r.json();
            if (d && d.ok) break;
      ...
      if (d && d.ok) {
        onNotify?.(`Bienvenido ${d.user?.nombre?.split(" ")[0]||""}`,"success");
        onLogin(d.user);
        setLoading(false);
        return;
      }
    ...
    const demoUser = {
      id: type === "admin" ? 1 : type === "rescatista" ? 2 : 3,
      nombre: type === "admin" ? "Admin DoGood" : type === "rescatista" ? "Refugio Demo" : "Carlos Adoptante",
      email: m.e,
      rol: type === "admin" ? "admin" : type === "rescatista" ? "rescatista" : "usuario"
    };
    onNotify?.(`¡Bienvenido ${demoUser.nombre}! 🐾`,"success");
    onLogin(demoUser);
    setLoading(false);
  };
  const doRegister=async()=>{
    if(!nombre||!regEmail||!regPass){const m="Completa todos los campos obligatorios";setErr(m);onNotify?.(m,"error");return;}
    setLoading(true);setErr("");
    ...
    // Guardado inmediato local para persistencia entre pestañas y sesiones
    try {
      let currentUsers = JSON.parse(localStorage.getItem("dogood_custom_users") || "[]");
      currentUsers = [newUserObj, ...currentUsers.filter(u => u.email?.toLowerCase() !== regEmail.toLowerCase())];
      localStorage.setItem("dogood_custom_users", JSON.stringify(currentUsers));
    } catch {}

    try {
      const tryUrls = [
        "http://localhost:8000/api/auth.php?action=register",
        apiUrl("auth.php?action=register")
      ];
      for (const url of tryUrls) {
        try {
          const r = await fetch(url, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ nombre, email: regEmail, password: regPass, rol: "rescatista", telefono: tel, estatus: "pendiente" })
          });
          if (r.ok) {
    ...
    setWelcomeUser(newUserObj);
    onNotify?.(`¡Solicitud enviada! Tu registro como Rescatista está en revisión.`, "success");
    setLoading(false);
  };
  ...
  return(
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,background:"rgba(44,26,14,.65)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(10px)"}}>
      <div style={{background:C.cream,borderRadius:28,width:"100%",maxWidth:480,boxShadow:"0 32px 80px rgba(0,0,0,.3)",overflow:"hidden",animation:"fadeUp .3s ease"}}>
        ...
        {welcomeUser ? (
          <div style={{padding:"28px 28px 30px",textAlign:"center",animation:"fadeUp .3s ease"}}>
            <div style={{fontSize:"3.4rem",marginBottom:10}}>⏳ 🐾</div>
            <h3 style={{fontFamily:"'Fredoka',sans-serif",fontWeight:700,fontSize:"1.65rem",color:C.cafe,lineHeight:1.15,marginBottom:8}}>
              ¡Solicitud de Rescatista Registrada!
            </h3>
            <p style={{fontSize:".9rem",color:C.sub,lineHeight:1.6,marginBottom:20}}>
              Gracias <strong>{welcomeUser.nombre}</strong> por querer unirte a DoGood.<br/>
              Tu cuenta de <strong>Rescatista / Refugio</strong> se encuentra <strong>pendiente de aprobación por el Administrador</strong>.
            </p>
            <div style={{background:"#FFF7DA",borderRadius:14,padding:"16px",marginBottom:20,textAlign:"left",border:`1.5px solid #F0C21D`}}>
              <div style={{fontSize:".74rem",fontWeight:800,color:C.cafeLt,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Estado de tu registro</div>
              <div style={{fontSize:".88rem",color:C.ink,fontWeight:800}}>📧 Correo: {welcomeUser.email}</div>
              <div style={{fontSize:".82rem",color:"#B45309",fontWeight:700,marginTop:4}}>⏳ Estado: Pendiente de aprobación por el Admin</div>
              <div style={{fontSize:".78rem",color:C.sub,marginTop:6,lineHeight:1.4}}>
                Una vez que el Administrador autorice tu cuenta, recibirás tu correo de bienvenida y podrás iniciar sesión para publicar tus mascotas.
              </div>
            </div>
            <button
              onClick={onClose}
              className="paw-btn"
              style={{width:"100%",padding:"14px",border:"none",borderRadius:14,background:C.cafe,color:C.white,fontWeight:900,fontSize:".96rem",cursor:"pointer",boxShadow:`0 10px 24px ${C.shadowMd}`}}
            >
          ...
          <div style={{padding:"22px 28px 28px"}}>
            <div style={{display:"flex",gap:10,marginBottom:20,borderBottom:`2px solid ${C.beigedk}`,paddingBottom:10}}>
              <button onClick={()=>setTab("login")} style={{background:"none",border:"none",fontWeight:800,fontSize:".95rem",color:tab==="login"?C.cafe:C.muted,cursor:"pointer",borderBottom:`3px solid ${tab==="login"?C.cafe:"transparent"}`,paddingBottom:6}}>
                Iniciar sesion
              </button>
              <button onClick={()=>setTab("register")} style={{background:"none",border:"none",fontWeight:800,fontSize:".95rem",color:tab==="register"?C.cafe:C.muted,cursor:"pointer",borderBottom:`3px solid ${tab==="register"?C.cafe:"transparent"}`,paddingBottom:6}}>
                Crear cuenta
              </button>
            </div>
          ...
          {tab==="login"?(
            <>
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  borderRadius: 12,
                  background: loading ? C.muted : C.cafe,
                  color: C.white,
                  fontWeight: 900,
                  fontSize: ".96rem",
                  cursor: loading ? "default" : "pointer",
                  boxShadow: `0 8px 20px ${C.shadowMd}`
                }}
              >
              ...
              <div style={{marginTop:16,background:C.beige,borderRadius:12,padding:"12px 14px"}}>
                ...
                {[["admin","Admin - dogood@teotek.com.mx"],["rescatista","Rescatista - refugio@dogood.mx"]].map(([t,l])=>(
                  <button key={t} type="button" onClick={()=>quickLogin(t)} style={{display:"flex",width:"100%",padding:"9px 12px",marginBottom:6,background:C.white,border:`1.5px solid ${C.beigedk}`,borderRadius:10,fontSize:".82rem",cursor:"pointer",color:C.ink,fontWeight:700,transition:"border-color .15s"}}
            ...
            <>
              {[["Nombre completo / Refugio","text",nombre,setNombre,"Ej. Refugio Huellitas"],["Correo electrónico","email",regEmail,setRegEmail,"tu@correo.com"],["Teléfono (Sólo números)","tel",tel,v=>setTel(v.replace(/\D/g,"")),"5512345678"],["Contraseña","password",regPass,setRegPass,"........"]].map(([l,t,v,fn,ph])=>(
                <div key={l} style={{marginBottom:12}}>
                  <label style={{display:"block",fontSize:".76rem",fontWeight:800,color:C.sub,textTransform:"uppercase",letterSpacing:.6,marginBottom:6}}>{l}</label>
                  <input type={t} inputMode={t==="tel"?"numeric":"text"} pattern={t==="tel"?"[0-9]*":undefined} value={v} onChange={e=>fn(e.target.value)} placeholder={ph}
                    style={{width:"100%",padding:"12px 14px",border:`1.5px solid ${C.beigedk}`,borderRadius:12,fontSize:".9rem",color:C.ink,outline:"none",background:C.white}}
                    onFocus={e=>e.target.style.borderColor=C.cafe} onBlur={e=>e.target.style.borderColor=C.beigedk}/>
                </div>
              ))}
              <div style={{marginBottom:14,padding:"10px 14px",background:"#FFF7DA",borderRadius:10,border:"1px solid #F0C21D",fontSize:".8rem",color:"#92400E",fontWeight:700}}>
                🐕 Registro exclusivo para Rescatistas / Refugios (Tu cuenta quedará pendiente de aprobación por el Administrador).
              </div>
              <button onClick={doRegister} disabled={loading} className="paw-btn" style={{width:"100%",padding:"13px",border:"none",borderRadius:12,background:loading?C.muted:C.cafe,color:C.white,fontWeight:900,fontSize:".95rem",cursor:loading?"default":"pointer",boxShadow:`0 8px 20px ${C.shadowMd}`}}>
